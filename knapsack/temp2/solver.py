...
def solveIt(inputData):
    ...
    while (1):
        ...
        if end == False and take == True and node[2]>cur_estimate :
            successors.append([node[0],node[1],node[2]-values[ratio[node[3]][1]],node[3]+1,0])
...
def calc_estimate(taken,Capacity):
    estimate = 0
    chk=0
    #print taken
    for i in range(0,len(ratio)):
        if i < len(taken):
            ...
            if taken[i] == 1:
                if (chk+weights[ratio[i][1]]) <= Capacity:
                    estimate = estimate + values[ratio[i][1]]
                    chk=chk+weights[ratio[i][1]]
                else:
                    #print "yes"
                    estimate = estimate + ratio[i][0]*(Capacity-chk)
                    break
        else:
            estimate = estimate + ratio[i][0]*(Capacity-chk)
            break
