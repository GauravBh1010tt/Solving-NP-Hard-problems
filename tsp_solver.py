...
def sim_annealing(path,temp = 1.99998,iteration=5000):
    mini = cal_dis(path)
    min_path = path
    #temp = 1.99998
...
def solve(data,temp,iteration):
    ploting(data)
    path=sim_annealing(data,temp,iteration)
    ploting(path)
    return cal_dis(path),path
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 1:
        ...
        if len(sys.argv)>2:
            temp = float(sys.argv[2].strip())
            iteration = int(sys.argv[3].strip())
        else:
            temp, iteration = 1.99998, 5000
        ...
        print solve(points,temp,iteration)
    else:
